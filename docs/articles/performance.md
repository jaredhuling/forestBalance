...
4.  **Weight computation** (`kernel_balance`) – adaptive solver:
    - **Direct**: sparse Cholesky on treated/control sub-blocks of
      `kern`. Preferred for smaller problems.
    - **CG**: conjugate gradient using the factored $`Z`$
      representation. No kernel matrix is formed. Preferred for large
      $`n`$. Requires `Z` and `num.trees`.
    - **BJ**: Block Jacobi preconditioned CG. Additionally requires
      `leaf_matrix`. Falls back to CG if `leaf_matrix` is not provided.
...
of iterations and the sparse matrix–vector product size. The C++ stages
(leaf extraction and $`Z`$ construction) remain a small fraction
throughout.
